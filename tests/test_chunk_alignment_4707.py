...
# Canonical button addresses of the install (from the same production
# log: every one of these decodes on at least one ALIGNED module).
INSTALL_INVENTORY = {
...
def test_4707_realigns_against_inventory_and_recovers_real_records() -> None:
    """THE FIX: with the install inventory available, the first-frame
    phase detection re-aligns the walk — the module's real records
    decode and the production phantoms are gone."""
    ...
    assert REAL_BUTTONS <= buttons, (
        f"real buttons missing after re-alignment: {sorted(buttons)}"
    )
    assert not (PRODUCTION_PHANTOMS & buttons), (
        f"phantoms survived re-alignment: {PRODUCTION_PHANTOMS & buttons}"
    )
    # The full table comes back: the symmetric 1CFBA0/1DF256 'general
    # off' pairs across six channels, plus the per-channel M01 links.
    assert len(decoded) >= 25


def test_4707_without_inventory_keeps_current_behaviour() -> None:
    """BOUNDED: with no inventory to score against (bare harness, fresh
    install before any button discovery), the walk is unchanged — the
    misalignment reproduces exactly as in the production log. The fix
    never guesses without evidence."""
...
def test_9105_control_group_decodes_real_buttons() -> None:
    ...
    # And the aligned table is NOT re-aligned (phase 0 wins its own score).
