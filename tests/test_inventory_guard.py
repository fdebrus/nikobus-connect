"""Unit tests for ``is_known_button_canonical``.

The decoder-side inventory gate was removed when the alt-alignment
chunking was reverted to 0.8.0's single-alignment walk — phantoms
were a symptom of misaligned windows, not of records pointing to
buttons absent from inventory. The helper itself is still defined
in ``protocol.py`` for external callers and the few merge-layer
sites that use the 8-channel +1 alias check; the tests below pin
that surface.

The decoder-integration cases that used to assert "the decoder
drops records pointing to unknown buttons" have been removed —
that behaviour no longer exists by design. The positive-path
decoder tests (real button decodes, 8-ch alias decodes) remain
below to pin that the simpler decoder still produces the right
...
from nikobus_connect.discovery import switch_decoder
...
def test_is_known_button_accepts_8ch_plus_one_alias():
    ...
    that case at merge time, so callers that consult this helper
    must not drop it.
    """
...
def test_is_known_button_rejects_phantom_address():
    """Canonicals matching no inventory entry (and no +1 alias) return False."""
...
def test_is_known_button_lenient_when_no_coordinator():
    ...
    addresses — the helper exists to filter against a *live* inventory,
    not to enforce one."""
...
# ---------------------------------------------------------------------------
# Decoder integration: positive-path decodes
# ---------------------------------------------------------------------------
...
def test_switch_decoder_keeps_8ch_plus_one_alias():
    """The +1 sibling of an 8-channel button must produce a real record.

    The decoder no longer rejects records based on inventory presence;
    this test verifies the +N key-nibble shift inside
    ``get_push_button_address`` still produces the right pair when
    the canonical is an 8-channel ``inventory_addr + 1`` sibling.
    """
