...
- **A state answer that arrives before our own ack is not ours.** A Feedback Module polls the output modules on its own, and the `$1C` answer to one of its queries — possibly for the other output group of the same module — could land while the command handler waited for the answer to *its* query, and be taken as it (the wrong group's six bytes displayed until the next poll). The handler now holds a state answer that precedes the interface's `$05xx` ack and uses it only when nothing fresher follows the ack, so a gateway that orders ack and answer differently keeps working. Clock and raw replies are unaffected.
- **The listener counts Feedback Module traffic.** `feedback_queries_seen` (`$1012` / `$1017` echoes, which only a PC-Link relays) and `feedback_answers_seen` (pushed `$1C` frames). Answers without queries mean the serial port is the Feedback Module's own, where its queries are invisible and its answers cannot be attributed to an output group: a caller can say so instead of mixing the groups. Both reset with `reset()`.
- **The presence probe is now the `#A` identity broadcast, and reports the gateway's identity.** The previous probe, a status query to the null address, only earned an acknowledgement — and the PC-Link holds acknowledgements back until it has a reply or a relayed bus frame to send, so on a quiet bus (no Feedback Module polling) the probe never got an answer and every start-up logged a false warning. `#A` is answered by the gateway itself, within tens of milliseconds, with its own `$18` status frame (captured: `$18F58600500F3FFFAC61FE` for a PC-Link at 86F5; a PC-Logic answers likewise). The probe reads it and exposes `NikobusConnect.gateway_address` and `gateway_family` (`pc_link`, `feedback_module`, `pc_logic`); any other Nikobus frame still counts as presence, so a gateway whose `#A` behaviour is unknown is not penalised while the bus is alive. One send per attempt, three attempts of three seconds.
- **Family signatures.** `const.FAMILY_SIGNATURES` maps each device class to the family byte of its `$18` status (switch 0x10 / compact 0x90, roller 0x20, dimmer 0x30, PC-Link 0x50, PC-Logic 0x40, Feedback Module 0xA0); `protocol.family_name()` and `protocol.family_matches()` let a caller check that a module answers as the type it is stored as.
- **PC-Link calendar channels are recognised in link records.** The Nikobus software gives the PC-Link 100 calendar channels (CH001 … CH100, halves A and B) that calendar programs and scenes fire like buttons; an output module's link record carries them as button addresses that no wall button ever matches, so those links were dropped as unmatched — and the wall-button address transform folds distinct channels together. The decoders now keep the channel's own address (`E00320` for CH001A) and label, and the merge files the link under a synthesized `PC-Link Calendar Channel` entry (`calendar_channel` field, one `CAL` operation point) so a host can show "Calendar channel CH001A (PC-Link)" among an output's triggers. Nothing is ever sent to such an entry: what the PC-Link puts on the bus when it fires a channel is not known.
...
- **Dimmer labels now read as the Nikobus software shows them.** The M11 / M12 preset level is the level on the dim controller's 1–10 V scale, `1,0 V … 10,0 V` (vendor tokens `S_D1 … S_D10`, rendered through the software's own language table); an earlier reading had turned them into `1% … 10%`. The M01 / M02 / M03 "dimming speed on / off" setting carries the vendor's wording (`T2=Dimming time on ; Dimming time off=1s`, …) instead of an invented `On/off step n`. `DIMMER_AMOUNT_PERCENT` remains as an alias of the new `DIMMER_PRESET_LEVEL`.
...
- **A connection now checks that a Nikobus device is on the other end.** The handshake writes its eight commands blindly, so a wrong port, an unpowered PC-Link, a serial bridge with nothing behind it or a handle left dead by the Nikobus PC software all "completed" it, and the failure only surfaced as timeouts later. `connect()` now follows the handshake with a presence probe: a status query to the null address, which the PC-Link and a feedback module used as gateway acknowledge with `$0511`; any other well-formed Nikobus frame seen meanwhile (a button press, a feedback frame, an ack) counts too, so a gateway whose null-address behaviour is unknown (a PC-Logic) is not penalised while the bus is alive. The interface holds an acknowledgement until the next `$` frame it receives — visible in every start-up log, where the `$0511` arrives with the first command's reply — so each attempt sends the probe twice and reads after the second; three attempts of three seconds. Silence is **not** fatal: the connection stays up, a warning says what to check, and the verdict is exposed as `NikobusConnect.device_answered` for the integration to surface.
- **Decoder tables pinned against the vendor's own.** `tests/fixtures/vendor_param_tables.json` holds the `ParamBase` value lists and the per-mode parameter assignments (`LinkModeBase`) of a PC-software project file, plus the English display strings of the software's language table; a test checks every switch, roller and dimmer table in `mapping.py` and every mode → parameter dispatch in the decoders against them.
...
**Feedback module read validated on hardware; made resilient.**
