...
- **A connection now proves a Nikobus device is on the other end.** The handshake writes its eight commands blindly, so a wrong port, an unpowered PC-Link, a serial bridge with nothing behind it or a handle left dead by the Nikobus PC software all "completed" it, and the failure only surfaced as timeouts later. `connect()` now follows the handshake with a presence probe: a status query to the null address, which the PC-Link (and a feedback module used as gateway) acknowledges with `$0511`. The interface holds that acknowledgement until the next `$` frame it receives — visible in every start-up log, where the `$0511` arrives together with the first command's reply — so each attempt sends the probe twice and reads after the second; three attempts of three seconds, then `NikobusConnectionError` with what to check. The transport is closed on failure, as after a failed handshake, so Home Assistant's setup retry and the config flow's connection test both see a real answer instead of an open port.
