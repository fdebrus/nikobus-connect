...
- **The presence probe reports the gateway's identity.** The gateway answers the null-address status query with its own `$18` status frame, usually right behind the ack; the probe now reads it and exposes `NikobusConnect.gateway_address` (e.g. `86F5`) and `gateway_family` (`pc_link`, `feedback_module`, `pc_logic`). Best effort: a gateway that sends no such frame leaves both `None`.
